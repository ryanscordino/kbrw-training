...
var Layout = createReactClass({
  ...
  loader() {
    const callback = () => { this.setState({ loader: null }) }
    this.setState({
      loader: {
        callback
      }
  ...
  render() {
    ...
    return <JSXZ in="orders">
      <Z sel=".layout-container">
        <Child {...props} />
        {/* <this.props.Child {...this.props} /> */}
        <JSXZ in="modal" sel=".modal-wrapper" className={cn(classNameZ, { 'hidden': !modal_component })}>
          {modal_component}
        </JSXZ>
        <JSXZ in="loader" sel=".loader-container" className={cn(classNameZ, { 'hidden': !this.state.loader })}>
          <h1>LOADING...</h1>
        </JSXZ>
      </Z>
    </JSXZ>
...
var Orders = createReactClass({
  ...
  render() {
    ...
    return <JSXZ in="orders" sel=".body-div">
      <Z sel=".table" in=".body-div">
        {/* JSXZ and Z should alternate */}
        <JSXZ in="orders" sel=".table-header"></JSXZ>
        {
          this.props.orders.value.map((order) => {
            ...
            return (
              <JSXZ in="orders" sel=".table-row" key={order.id}>
                ...
                <Z sel=".name">{custom.customer.full_name}</Z>
                <Z sel=".address">
                ...
                <Z sel=".quantity">{custom.items.length}</Z>
                ...
                <Z sel=".div-body-table">
                  <JSXZ in="orders" sel=".button-pay">
                  </JSXZ>
                  <JSXZ in="orders" sel=".div-block-6">
                    {`Status : ${payment.account_status}`}
                  </JSXZ>
                  <JSXZ in="ordersTable" sel=".div-block-6">
...
var Order = createReactClass({
  ...
  render() {
    ...
    const custom = order.custom
    return <JSXZ in="order-detail" sel=".body-div">
          ...
          custom.items.map((item) => {
            return (
              <JSXZ in="order-detail" sel=".order-row" key={item.item_id}>
                ...
                <Z sel=".product-total">{item.price * item.quantity_to_fetch}</Z>
              </JSXZ>)
          })
        }
      </Z>
...
var ErrorPage = createReactClass({
  render() {
    return <h1>YO! You shouldn't be here</h1>
  }
})
...
function reload(remoteProp) {
  ...
  addRemoteProps(browserState).then(
    (props) => {
      ...
      ReactDOM.render(<ErrorPage message={"Something happened"} code={res.http_code} />, document.getElementById('root'))
    })
}
